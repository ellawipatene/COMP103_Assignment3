...
public class HospitalERCore{
    ...
    public Queue<Patient> waitingRoom = new ArrayDeque<Patient>();
    public static final int MAX_PATIENTS = 5;   // max number of patients currently being treated
    public Set<Patient> treatmentRoom = new HashSet<Patient>();
        ...
        while (running){         // each time step, check whether the simulation should pause.
            ...
            for (Patient p: treatmentRoom){
                ...
                if(p.completedCurrentTreatment()){
                    ...
                    if(p.getPriority() == 1){
                        priority_processed++; 
                        priority_processed_time = priority_processed_time + p.getTreatmentTime();
                    }
                    total_processed_time = total_processed_time + p.getTreatmentTime();
                    total_processed++;
